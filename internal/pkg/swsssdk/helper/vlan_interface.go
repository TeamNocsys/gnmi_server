...
type VlanInterface struct {
    ...
    Data *sonicpb.AcctonVlan_VlanInterface_VlanInterfaceList
}
...
func (c *VlanInterface) LoadFromDB() error {
    ...
    // 获取配置信息
    if c.Data == nil {
        c.Data = &sonicpb.AcctonVlan_VlanInterface_VlanInterfaceList{}
    }
...
func (c *VlanInterface) SaveToDB(replace bool) error {
    ...
    if replace {
        if _, err := conn.SetEntry("VLAN_INTERFACE", c.Key, e); err != nil {
            return err
        }
    } else {
        if _, err := conn.ModEntry("VLAN_INTERFACE", c.Key, e); err != nil {
            return err
        }
    }
...
func (c *VlanInterface) RemoveFromDB() error {
    conn := c.Client.Config()
    if conn == nil {
        return swsssdk.ErrConnNotExist
    }
