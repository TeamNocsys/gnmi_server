...
type InterfaceIPPrefix struct {
    ...
    Data *sonicpb.SonicInterface_Interface_InterfaceIpprefixList
}
...
func (c *InterfaceIPPrefix) LoadFromDB() error {
    ...
    // 获取配置信息
    if c.Data == nil {
        c.Data = &sonicpb.SonicInterface_Interface_InterfaceIpprefixList{}
    }
    if data, err := conn.GetAll(swsssdk.CONFIG_DB, append([]string{"INTERFACE"}, c.Keys...)); err != nil {
        ...
        for k, v := range data {
            ...
            case "scope":
                switch v {
                case "local":
                    c.Data.Scope = sonicpb.SonicInterface_Interface_InterfaceIpprefixList_SCOPE_local
                case "global":
                    c.Data.Scope = sonicpb.SonicInterface_Interface_InterfaceIpprefixList_SCOPE_global
                }
            case "family":
                switch v {
                case "IPv4":
