...
type VlanInterfaceIPPrefix struct {
    ...
    Data *sonicpb.AcctonVlan_VlanInterface_VlanInterfaceIpprefixList
}
...
func (c *VlanInterfaceIPPrefix) LoadFromDB() error {
    ...
    // 获取配置信息
    if c.Data == nil {
        c.Data = &sonicpb.AcctonVlan_VlanInterface_VlanInterfaceIpprefixList{}
    }
    if data, err := conn.GetAll(swsssdk.CONFIG_DB, append([]string{"VLAN_INTERFACE"}, c.Keys...)); err != nil {
        ...
        for k, v := range data {
            ...
            case "scope":
                switch v {
                case "local":
                    c.Data.Scope = sonicpb.AcctonVlan_VlanInterface_VlanInterfaceIpprefixList_SCOPE_local
                case "global":
                    c.Data.Scope = sonicpb.AcctonVlan_VlanInterface_VlanInterfaceIpprefixList_SCOPE_global
                }
            case "family":
                switch v {
                case "IPv4":
